...
class PlayState extends MusicBeatState
{
	...
	private var agoti:Character;
	private var tabi:Character;
	...
	private var floatagoti:Float = 0;
	private var floattabi:Float = 0;
	...
	var agotiDad:Bool = false;
	var tabiDad:Bool = false;
	var isbf:Bool = false;
	...
	private var agotiTrail:FlxTrail;
	private var tabiTrail:FlxTrail;
	...
	override public function create()
	{
		...
		if (agotiDad)
		{
			agoti = new Character (-311, 169 /* idk */, "agoti");
		}

		if (tabiDad)
		{
			tabi = new Character (741, -42, "tabi");
		}
		...
		var dudewatermark = new  FlxText(4, FlxG.height - 36, 0, "Mod by dude0216", 16);
		...
		var simwatermark = new  FlxText(5, FlxG.height - 20, 0, "Modded Again by Similirity", 16);
		simwatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		simwatermark.scrollFactor.set();
		add(simwatermark);
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == "god-eaters")
			{
				new FlxTimer().start(0.002, function(cbt:FlxTimer)
				{
					if (ctrTime == 0)
		...
		startTimer = new FlxTimer().start(Conductor.crochet / (1000 / c_div), function(tmr:FlxTimer)
		{
			...
			if (agotiDad) agoti.dance();
			if (tabiDad) tabi.dance();
	...
	override public function update(elapsed:Float)
	{
		...
		if (godModeFloat)
		{
			...
			agoti.y += Math.sin(floatagoti);
			tabi.y += Math.sin(floattabi);
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && !songEnded)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				if (agotiDad == true)
					camFollow.setPosition(agoti.getMidpoint().x + 100, agoti.getMidpoint().y - 100);
					
				if (tabiDad == true)
					camFollow.setPosition(tabi.getMidpoint().x - 496, tabi.getMidpoint().y + 31);
		...
		if (health <= 0)
		{
			...
			if (Main.god)
			{
				...
				else if (agotiDad == true)
				{
					FlxG.sound.play(Paths.sound('fnf_loss_agoti'));
					FlxG.switchState(new PlayState());
				}
				else if (tabiDad == true)
				{
					FlxG.sound.play(Paths.sound('fnf_loss_tabi'));
					FlxG.switchState(new PlayState());
				}
			}
		...
		if (generatedMusic)
			{
					...
					{	
						...
						if (!daNote.mustPress && daNote.wasGoodHit)
						{
							...
							if (agotiDad) agoti.holdTimer = 0;
							if (tabiDad) tabi.holdTimer = 0;
			...
			switch (bState)
			{
				...
				case 1:
					...
					if (bTime >= 240)
					{
						...
						{
							thanks = new Alphabet(6, FlxG.height / 2 + 380, "SECRET IN FREEPLAY", true, false);
		...
		if (SONG.song.toLowerCase() == 'final-destination' && curStep != stepOfLast)
		{
            ...
            ease: FlxEase.expoOut
				...
				if (Main.god)
				{
					...
					agotiTrail = new FlxTrail(agoti, null, 5, 10, 0.1, 0.01);
					tabiTrail = new FlxTrail(tabi, null, 5, 10, 0.1, 0.01);
				}
				case 128:
					resetCharacters();
				...
				case 1152:
					...
					switchCharacter('agoti');
					switchCharacter('tabi');
					FlxTween.tween(FlxG.camera, {zoom: 0.75}, 3.3, {
            ease: FlxEase.expoOut
				});
				shakeCam = true;
				if (Main.god)
            ...
            ease: FlxEase.expoOut
				...
				case 1856:
					resetCharacters();
					switchCharacter('agoti'); //2
				case 1984:
					resetCharacters();
					switchCharacter('tabi'); //1
				case 2112:
					resetCharacters();
