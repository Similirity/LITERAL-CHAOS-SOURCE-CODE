...
class MainMenuState extends MusicBeatState
{
	...
	var bgch:FlxSprite;
	var diff = 1;
	var infoTxt:Array<String> = ['', ''];
	var info:FlxText;
	var pressnine:FlxText;
	...
	override function create()
	{
		...
		/* pressnine = new FlxText(0, 15, 500, "Press E to access secret song", 20);
		pressnine.scrollFactor.set();
		pressnine.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		pressnine.screenCenter(X);
		add(pressnine); */
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{
					#if linux
					Sys.command('/usr/bin/xdg-open', ["https://www.youtube.com/c/dude0216", "&"]);
					#else
					FlxG.openURL('https://www.youtube.com/c/dude0216');
					#end
				}
				else
				{
		...
		{
			FlxG.sound.play(Paths.sound('secretselect'));
			FlxG.save.data.wii = 0;
